...
def get_preprocess_models():
    ...
    apparel_ohe = pickle.load(open(os.path.join(preprocessing_model_path, 'apparel_ohe.pkl'), 'rb'))
    attribute_ohe = pickle.load(open(os.path.join(preprocessing_model_path, 'attribute_ohe.pkl'), 'rb'))
    return [[apparel_le, apparel_ohe], [attribute_le, attribute_ohe]]


def get_task_features(df, preprocess_models):
    features = []
    cols = ["apparel", "attribute"]
    for models, col in zip(preprocess_models, cols):
        ft = models[1].transform(models[0].transform([df[col]]).reshape(-1, 1))
        features.append(ft)
    feat = scipy.sparse.hstack(features)
    return feat
